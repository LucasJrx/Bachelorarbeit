...
Ziel des Business Case ist es, eine Entscheidungsgrundlage zu schaffen, die sowohl auf monetären Kennzahlen wie Kapitalwert, Amortisationsdauer oder internem Zinsfuß basiert als auch nicht-monetäre Faktoren berücksichtigt, etwa Montageaufwand, Designfreiheit oder Normenkonformität @bruggerITBusinessCase2009[Kap.7, 8]@taschnerBusinessCases2017[Kap.12.4].

Die Business Case Analyse stellt damit einen praxisorientierten Bewertungsrahmen dar, der über eine reine Kostenbetrachtung hinausgeht und eine fundierte Entscheidung hinsichtlich der Einführung flexibler Kupferschienen im industriellen Schaltschrankbau ermöglicht @taschnerBusinessCases2017[Kap.3].
...
In diesem Schritt werden sämtliche quantitativen und qualitativen Daten zusammengeführt, die für die spätere Bewertung der Handlungsalternativen erforderlich sind. Dazu zählen sowohl Kostenfaktoren (z.B. Material-, Montage- und Wartungskosten) als auch Nutzenkomponenten wie Zeitersparnis und Flexibilität. Die Datengrundlage umfasst interne Quellen, externe Angebote sowie praxisbasierte Erfahrungswerte. In den letzten 5 Jahren wurden im Mittel 25 H4 Module pro Jahr verkauft und 35,2 H6 Module. Bei Sonderkonstruktionen variiert die Stückzahl stark, im Durchschnitt werden jedoch ca. 15 solcher Projekte mit ähnlichen Anforderungen pro Jahr realisiert. Es wurden Lieferantenangebote für flexible Kupferschienen von der Firma nVent (@nVentpreise) und EMS (@emsangebot) eingeholt. Die Preise der aktuellen Schienen wurden von ABB bereitgestellt (@abbpreise). Aus den Preisen und Gewichten der Schienen lassen sich die Materialkosten pro Kilogramm berechnen.
...
Die Amortisationsrechnung hilft, das Risiko der Investition besser einzuschätzen. Sie zeigt, bei welcher Investition ABB am schnellsten wieder die Investitionskosten deckt und das eingesetzte Kapital zurückgewinnt. Zudem unterstützt sie ABB dabei, die Finanzen gezielt zu planen. Hierdurch erfährt man, wie lange das Kapital in einer bestimmten Investition gebunden bleibt @AmortisationsrechnungDefinitionFormeln. Daraus ergibt sich folgende Rechnunge für beide Lieferanten mit folgenden Variablen, die Investitionskosten setzten sich aus den Werkzeugkosten plus Schulungskosten für drei Mitarbeiter zusammen. Die jährlichen Rückflüsse setzen sich aus der differenz der Kosten der aktuellen Kupferpreisen und den Kupferpreisen des jeweiligen Lieferanten multipliziert mit der Anzahl der verkauften Module oder Sonderkonstruktionen @bruggerITBusinessCase2009[Kap. 7.1; 7.1.2].
...
Dieser berechnet sich, indem man die zurechenbaren Kosten der Investition von den zurechenbaren Erlösen abzieht @GewinnvergleichsrechnungEinfachErklaert @bruggerITBusinessCase2009[Kap. 7ff.]. Die Formel der Gewinnvergleichsrechnung lautet
...
Die Investition wird dabei anhand ihrer Rentabilität beurteilt und wählt diejenige mit dem höheren Wert. Man wählt dann diejenige Alternative mit dem höheren Wert  @RentabilitaetsrechnungFormelBeispiel @bruggerITBusinessCase2009[Kap. 7ff.].
...
Der Schulungsaufwand für die Einführung flexibler Kupferschienen ist im Vergleich zu starren Varianten höher. Während das Fertigungspersonal mit den neuen Montageprozessen vertraut gemacht werden muss, sind auch Anpassungen im Engineering erforderlich, um die neuen Designmöglichkeiten optimal zu nutzen. Dies umfasst sowohl theoretische Schulungen als auch praktische Übungen, um sicherzustellen, dass alle Mitarbeiter die neuen Verfahren und Werkzeuge sicher und effizient anwenden können. Trotz des anfänglich höheren Aufwands führt die Investition in Schulungen langfristig zu einer verbesserten Prozesssicherheit und Effizienz, da das Personal besser auf die Anforderungen und Herausforderungen der flexiblen Schienen vorbereitet ist. Zudem können durch gezielte Schulungsmaßnahmen Fehlerquellen wie z.B. falsches Biegen der Schienen minimiert und die Qualität der Endprodukte gesteigert werden.

Aus den vorher beschriebenen Punkten ergibt sich für die Nutzwertanalyse welche in Folgender @Nutzwerttabelle aufgeführt wird. Unter jeder Schienenoption wird eine Punktzahl links vergeben und rechts die gewichteten Punktwerte. Die Gewichtung der Kriterien erfolgt auf Basis ihrer Relevanz für die Gesamtbewertung.

#figure(
  caption: [Nutzwertanalyse],
  table(
    columns: (65pt, 80pt, auto, auto, auto),
    align: horizon,
    table.header(
      [*Kriterium*], [*Gewichtung*], [*ABB*], [*flexible\ Schienen*], [*Sonderkonstruktion*],
    ),
    // [], [],
     // [#table(columns: (auto, auto), stroke: none, [Punkte], table.vline(stroke: 0.6pt), [Gewichtung ])],
     // [#table(columns: (auto, auto), stroke: none, [Punkte], table.vline(stroke: 0.6pt), [Gewichtung ])],
     // [#table(columns: (auto, auto), stroke: none, [Punkte], table.vline(stroke: 0.6pt), [Gewichtung ])],

    [Layout-\ Flexibilität], [25%],
    [#table(columns: (auto, auto), stroke: none,  [1], table.vline(stroke: 0.6pt), [$0,25*1= 0,25$])],
    [#table(columns: (auto, auto), stroke: none,  [4], table.vline(stroke: 0.6pt), [$0,25*4= 1$])],
    [#table(columns: (auto, auto), stroke: none,  [2], table.vline(stroke: 0.6pt), [$0,25*2= 0,5$])],

    [Engineering], [25%],
    [#table(columns: (auto, auto), stroke: none,  [2], table.vline(stroke: 0.6pt), [$0,25*2= 0,5$])],
    [#table(columns: (auto, auto), stroke: none,  [3], table.vline(stroke: 0.6pt), [$0,25*3= 0,75$])],
    [#table(columns: (auto, auto), stroke: none,  [2], table.vline(stroke: 0.6pt), [$0,25*2= 0,5$])],

    [Schulungen], [15%],
    [#table(columns: (auto, auto), stroke: none,   [4], table.vline(stroke: 0.6pt), [$0,15*4= 0,6$])],
    [#table(columns: (auto, auto), stroke: none,  [1], table.vline(stroke: 0.6pt), [$0,15*1= 0,15$])],
    [#table(columns: (auto, auto), stroke: none,   [3], table.vline(stroke: 0.6pt), [$0,15*3= 0,45$])],

    [Montage], [35%],
    [#table(columns: (auto, auto), stroke: none,  [1], table.vline(stroke: 0.6pt), [$0,35*1= 0,35$])],
    [#table(columns: (auto, auto), stroke: none,   [4], table.vline(stroke: 0.6pt), [$0,35*4= 1,4$])],
    [#table(columns: (auto, auto), stroke: none,  [2], table.vline(stroke: 0.6pt), [$0,35*2= 0,7$])],

    [Summe], [100%],
    [#table(columns: (auto, auto), stroke: none,  [8], table.vline(stroke: 0.6pt), [1,7])],
    [#table(columns: (auto, auto), stroke: none,  [12], table.vline(stroke: 0.6pt), [3,3])],
    [#table(columns: (auto, auto), stroke: none,  [9], table.vline(stroke: 0.6pt), [2,15])],
  )
)<Nutzwerttabelle>
...
Die Wirkung zentraler Treiber auf NBW und Amortisation wird zusammengefasst @taschnerBusinessCases2017[Kap.11.3].
