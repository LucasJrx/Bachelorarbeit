#let check-attributes(
  ...
  let boolean-attributes = (
    ...
    show-formula-list: show-formula-list,
  ...
  if (processing_period != none and (type(processing_period) != str or processing_period.len() == 0)) {
  panic("Processing period is invalid. Specify a string in the 'processing_period' attribute of the template.")
}
